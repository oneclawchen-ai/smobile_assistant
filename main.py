...
def get_vision_ai_response(img_path):
    try:
        ...
        vision_prompt = (
            "你是一位資深的行動通訊維運專家。請仔細讀取這張網管系統截圖或告警簡訊（包含任何 OCR 文字）：\n"
            "1. 【動態情境解析】：絕對不要套用固定模板。請務必精確擷取圖片中真實呈現的「站台名稱」、「異常人數」、「特定 KPI（如 RRC成功率、PRB負載等）」。判斷這是單一站台斷線故障，還是大型活動的熱區壅塞。\n"
            "2. 【白話文診斷】：將冷冰冰的數據轉化為一線人員秒懂的「現況解讀」。明確說明這些數據對用戶體驗的影響（例如：PRB 飆高代表用戶正經歷降速；RRC 歸零代表該區完全斷線）。\n"
            "3. 【SOP 與參數優化建議】：請參考大型活動或基站維運 SOP（如：站台信號涵蓋調整、Idle Reselection、Pcell Swap 或 HO 參數調整等機制），針對圖片中的具體問題，給出對應的「參數調整（如天線傾角、pmax）」或「負載分流」實作建議。\n"
            "4. 【排版規範】：請全程使用繁體中文 (zh-TW)，採層次分明的條列式排版，絕對不要使用 Markdown 表格。內容需簡潔直接，總字數嚴格控制在 800 字以內。\n"
            "5.  如果沒有以上的相關資訊，就自行分析圖片做說明。\n"
            "6. 回覆結尾請換行並獨立加上：『📡 溫馨提醒：以上數值解析由 AI 輔助生成，實際調整請依據網管中心最新 SOP 執行喔！』"
        )
